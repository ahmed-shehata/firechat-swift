...
class LoginViewController : UIViewController, UIActionSheetDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        ref = Firebase(url:"https://<YOURURL>.firebaseio.com")
        authHelper = TwitterAuthHelper(firebaseRef: ref, twitterAppId: "KEY")
    }
    ...
    @IBAction func login(sender: UIButton) {
        ref.createUser("test@email.com", password: "supersecurepassword") { error -> Void in
            
            self.ref.authUser("test@email.com", password: "supersecurepassword") {
                error, authData in
                if error != nil {
